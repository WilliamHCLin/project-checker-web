...
def _parse_json(raw: str) -> dict:
    ...
    # 第三次：items 未閉合，找最後完整 item 的 } 補 ] 收尾
    last_item = candidate.rfind('},')
    if last_item == -1:
        last_item = candidate.rfind('}')
    if last_item != -1:
        repaired = candidate[:last_item+1] + '], "overall_comment": "（AI 輸出被截斷，以下為部分結果）"}'
        try:
            return json.loads(repaired)
        except json.JSONDecodeError:
            pass
...
G. 專案管理基本功 是通用底層，如果文件是具體的活動/比賽/班級規劃，請優先選更具體的場景。
...
- 若文件中的內容可以合理推論出某項目已被覆蓋，請標記為已達標，並在 evidence 中說明推論依據
- 不適用於本份文件的項目，請標記為「不適用」並簡短說明

第三步：謹慎判定缺失
- 只有在整份文件完全找不到相關資訊，且無法合理推論時，才標記為「未達標」或「缺失資訊」
